...
const setOpeningBalance = async (req, res, next) => {
  try {
    ...
    const { amount, usd_amount, date } = req.body;
    if (amount === undefined || amount === null) throw { statusCode: 400, message: 'Amount required' };
    const result = await accountsService.setOpeningBalance(req.params.id, parseFloat(amount), usd_amount, req.user.id, date || null);
