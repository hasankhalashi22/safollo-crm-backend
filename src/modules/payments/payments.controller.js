...
const updateMethod = async (req, res, next) => {
  try {
    const result = await updatePaymentMethod(req.params.id, req.body.payment_method, req.user.id);
    res.json({ success: true, ...result, message: 'Payment method আপডেট হয়েছে' });
  } catch (err) { next(err); }
};
